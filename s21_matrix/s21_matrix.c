...
int s21_create_matrix(int rows, int columns, matrix_t *result) {
    ...
    if (rows > 0 && columns > 0) {
        ...
        double **matrix = (double **)calloc(rows, sizeof(double *));  //  выделяем память на указатель
...
int s21_eq_matrix(matrix_t *A, matrix_t *B) {
    ...
    if (CHEK_CORECT && A->rows == B->rows && A->columns == B->columns) {
        for (int i = 0; i < A->rows; i++) {
            int k = 0;
            while (k < A->columns) {
                if (fabs(A->matrix[i][k] - B->matrix[i][k]) > EPS)
                                             err_status = FAILURE;  //  EPS 0,0000001 fabs = abc
                k++;
            }
        }
    } else {
